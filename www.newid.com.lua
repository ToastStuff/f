...
function Library.WindowTheme(themeName)
    ...
    for _, frame in pairs(game.Players.LocalPlayer.PlayerGui:GetDescendants()) do
        if frame:IsA("Frame") or frame:IsA("TextButton") or frame:IsA("TextLabel") then
            -- Update background colors
            if frame.Name == "Divider" then
                frame.BackgroundColor3 = theme.DividerColor
            elseif frame:IsA("Frame") then
                frame.BackgroundColor3 = theme.BackgroundColor
            elseif frame:IsA("TextButton") then
                frame.BackgroundColor3 = theme.ButtonColor
