...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    function Library.NewDivider()
        ...
        if lastButton then
            -- Position the divider just below the last button with more space
            Divider.Position = UDim2.new(0, 1, 0, lastButton.Position.Y.Offset + lastButton.Size.Y.Offset + buttonSpacing)
        else
    ...
    function Library.NewButton(text, callback)
        ...
        local lastDivider = nil  -- Track the most recent divider
        for _, child in ipairs(Frame:GetChildren()) do
            if child:IsA("Frame") and child.BackgroundColor3 == Color3.fromRGB(55, 53, 55) then
                lastDivider = child  -- Find the last divider
            end
        end
    
        -- Position the button under the last divider or at the top if no divider
        local newY
        if lastDivider then
            -- Position after the last divider
            newY = lastDivider.Position.Y.Offset + lastDivider.Size.Y.Offset + buttonSpacing
        else
            -- No divider, position at the top
            newY = 50
        end
    
        NewButton.Position = UDim2.new(0, 10, 0, newY)  -- Place below the divider or at the top
