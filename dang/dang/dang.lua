...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    function Library.NewButton(text, callback)
        ...
        -- Always position the button on the left side
        local newX = 10  -- Set all buttons to the left
    
        -- Calculate the Y position dynamically
        local lastDivider = nil
        for _, child in ipairs(Frame:GetChildren()) do
        ...
        local newY
        if lastDivider then
            -- Position button just below the divider
            newY = lastDivider.Position.Y.Offset + lastDivider.Size.Y.Offset + buttonSpacing
        else
            -- Default if no divider, position after title
            newY = 50 + (#buttons * (buttonHeight + buttonSpacing))  -- Dynamically adjust based on button count
        end
