...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    function Library.NewButton(text, callback)
        ...
        local newX = column == 0 and 10 or (windowWidth - buttonWidth - 5)  -- Align left or rightmost
    
        -- If the divider exists, position the new button below it
        ...
        -- If divider exists, position button below it
        local newY
        if lastDivider then
            newY = lastDivider.Position.Y.Offset + lastDivider.Size.Y.Offset + buttonSpacing
        else
            newY = 30 + (row * (buttonHeight + buttonSpacing))  -- Default if no divider
        end
