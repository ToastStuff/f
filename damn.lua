...
function Library.NewWindow(title)
  ...
  -- Add a variable to track the state of the ScreenGui
local isScreenGuiHidden = false
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.F1 then
        -- Only show the notification if the ScreenGui was hidden
        if isScreenGuiHidden then
            ...
            createNotification("Interface Shown!")
            
            -- Reset the flag to indicate the ScreenGui is no longer hidden
            isScreenGuiHidden = false
        end
    end
end)
