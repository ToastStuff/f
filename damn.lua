...
function Library.NewDivider()
    ...
    local topSpacing = 20 -- Space above the divider
    if lastButton then
        -- Position the divider below the last button with top spacing
        Divider.Position = UDim2.new(0, 1, 0, lastButton.Position.Y.Offset + lastButton.Size.Y.Offset + buttonSpacing + topSpacing)
    else
        -- If no buttons exist, place the divider below the title with top spacing
        Divider.Position = UDim2.new(0, 1, 0, 40 + topSpacing)
    end

    Divider.Parent = Frame

    -- Add space below the divider for buttons underneath
    local bottomSpacing = 20
    local dividerIndex = #buttons + 1
    buttons[dividerIndex] = Divider
    Divider:SetAttribute("BottomSpacing", bottomSpacing)
